 ...
 */
public class BlockAdapter extends BaseQuickAdapter<NotificationBean,BaseViewHolder> {

    private Context mContext =null;
    private boolean isOpen =false;
    public BlockAdapter(Context context) {
        super(R.layout.settingapp_item_block_layout);
    ...
    protected void convert(BaseViewHolder helper, final NotificationBean item) {
        ...
        if(item!=null){
            ...
            if(!TextUtils.isEmpty(item.getAppName())){
                helper.setText(R.id.settingapp_block_name,item.getAppName()+"");
            }

            ImageView icons =helper.getView(R.id.settingapp_block_icon);
            ...
            Switch selectApp =helper.getView(R.id.settingapp_block_cb);
            if(isOpen){
...
//                selectApp.setFocusable(true);
            ...
            helper.addOnClickListener(R.id.settingapp_block_cb);
            if(item.isOpen()){
